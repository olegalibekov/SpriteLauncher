...
def save_img_old(res, imgid, divide=False):
...
def save_img(res, imgid, divide=False):
    ...
    output = (output * 255).astype('uint8')
    output = cv2.resize(output, origin_shape)
    # print(cv2.cvtColor(output,cv2.COLOR_RGB2RGBA))
    # bgr = cv2.cvtColor(cv2.cvtColor(output, cv2.COLOR_BGR2GRAY), cv2.COLOR_GRAY2BGR)

    # bgr = output[:, :, :3]  # Channels 0..2
    # gray = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY)
    # bgr = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)
    # alpha = output[:, :, 2]  # Channel 3
    ...
    cv2.imwrite('output/generated' + str(imgid) + '.png', dst)
...
canvas_array = []
final_canvas = torch.zeros([1, 3, width, width]).to(device)
